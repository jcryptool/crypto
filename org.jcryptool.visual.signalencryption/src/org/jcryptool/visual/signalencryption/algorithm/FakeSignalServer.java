...
/**
 * A class which acts similar to a real signal server. Allows to upload a
 * PreKeyBundle along with one-time-pre-keys. When initiating a communication
 * any entity can come to this interface and retrieve a bundle. Delivered
 * one-time-keys are deleted by the "server".
 */
public class FakeSignalServer {
    ...
    /**
     * Stores a partially initialized bundleTemplate along with the given
     * oneTimePreKeys.
     * <p/>
     * Any other communication party can retrieve them now via
     * {@link #retrievePreKeyBundle(SignalProtocolAddress)} and chat with you.
     */
    ...
    /**
     * Gets a KeyBundle for a given address. Obviously this "server" is trustworthy,
    ...
    public static PreKeyBundle retrievePreKeyBundle(SignalProtocolAddress address) {
        ...
        // the one-time-key may stay null, which is fine for the protocol. It's a bonus
        // key.
        return preKeyBundleStore.get(address).fill(oneTimePreKeyId, oneTimePreKey);
    }

    /**
     * The same as {@link #retrievePreKeyBundle(SignalProtocolAddress)} but it does
     * not delete the keys. This is required for UI purposes.
     */
    ...
    /**
     * Just for reference that clients should upload new one-time-pre-keys from time
     * to time.
     */
    public void publishOneTimePreKeys(SignalProtocolAddress address,
            SortedMap<Integer, ECPublicKey> freshOneTimePreKeys) {
