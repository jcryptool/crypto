 ...
 * @author Holger Friedrich (support for Eclipse Commands -- now derived from
 *         new class AbstractNewKeyStoreEntryHandler based on the old
 *         AbstractNewKeyStoreEntryAction, also added a dummy implementation of
 *         the execute() method)
 *
 */
public class IdentityManager extends AbstractNewKeyStoreEntryHandler {
	...
	/**
	 * TODO Where and when has the run() or execute() method been lost? Adding a
	 * dummy implementation to work around the compiler error -- but a meaningful
	 * implementation might actually be needed
	 */
	@Override
	 ...
	 * @param keyLength
	 */
	public LinkedList<KeyStoreAlias> createIdentity(final String name, final String algorithm, final String password, final int keyLength) {
...
//		staticImpl(null, name, algorithm, password, keyLength, argument, nkd);
			...
			if (gen != null && name != null && password != null && keyLength > 0) {
				...
				if (keyLength != -1) {
					if (gen.getParameterSpecClassName() != null) {
						spec = Reflector.getInstance().instantiateParameterSpec(gen.getParameterSpecClassName(),
								argument);
					}
				}
				...
				KeyStoreAlias publicAlias = performNewKeyAction(new NewKeyPairDescriptor(nkd, priv, pub));
				KeyStoreAlias privatAlias = KeyStoreManager.getInstance().getPrivateForPublic(publicAlias);
				generatedAliases.add(publicAlias);
				generatedAliases.add(privatAlias);
			} else {
				LogUtil.logInfo("Attention: One of the given parameters is wrong! Identity name: '" + name
						+ "', algorithm name: '" + algorithm + "', password: '" + password + "' and key length: '"
						+ keyLength + "'. Key creation failed!");
			}
	 ...
	 */
	public LinkedList<KeyStoreAlias> createMpRSAIdentity(final String name, final String password, final int keyLength,
	 ...
	 * @param password     for the private key
	 * @param crackedBy    specifies the name of the identity, who has cracked the
	 *                     key. If the key hasn't been cracked, enter 'null'!
	 * @param modulus      N
	 * @param firstPrime   p
	 * @param secondPrime  q
	 * @param pubExponent  e
	 ...
	 */
	public LinkedList<KeyStoreAlias> saveRSAKeyToKeystore(final String name, final String password,
			final String crackedBy, final BigInteger modulus, final BigInteger firstPrime, final BigInteger secondPrime,
		...
		if (crackedBy != null) {
			publicAlias = new KeyStoreAlias(name, KeyType.KEYPAIR_PUBLIC_KEY,
					getConcreteAlgorithm("RSA") + " (cracked by: " + crackedBy + ") ",
		...
		final RSAPrivateCrtKey privkey = new RSAPrivateCrtKey(n, e, new FlexiBigInt(privExponent),
				new FlexiBigInt(firstPrime), new FlexiBigInt(secondPrime), FlexiBigInt.ZERO, FlexiBigInt.ZERO,
				FlexiBigInt.ZERO);
		final KeyStoreAlias privateAlias;
		if (crackedBy != null) {
			privateAlias = new KeyStoreAlias(name, KeyType.KEYPAIR_PRIVATE_KEY,
					getConcreteAlgorithm("RSA") + " (cracked by: " + crackedBy + ") ",
					new BigInteger(modulus.toString()).bitLength(), (name.concat(modulus.toString())).hashCode() + "",
...
//		KeyStoreManager.getInstance().add
		...
		KeyStoreManager.getInstance().addCertificate(CertificateFactory.createJCrypToolCertificate(pubkey),
				publicAlias);

		// Return the created keyStoreAliases to show the key names in the GUI.
		LinkedList<KeyStoreAlias> aliases = new LinkedList<>();
		aliases.add(privateAlias);
		aliases.add(publicAlias);
		return aliases;
	}
	 ...
	 * @param privExponent d
	 */
	public LinkedList<KeyStoreAlias> saveMpRSAKeyToKeystore(final String name, final String password,
			final String crackedBy, final int primeNumber, final BigInteger modulus, final BigInteger firstPrime,
			final BigInteger secondPrime, final BigInteger thirdPrime, final BigInteger fourthPrime,
			final BigInteger fifthPrime, final BigInteger pubExponent, final BigInteger privExponent) {
		...
		if (crackedBy != null) {
			publicAlias = new KeyStoreAlias(name, KeyType.KEYPAIR_PUBLIC_KEY,
					"MpRSA" + " (cracked by: " + crackedBy + ") ", new BigInteger(modulus.toString()).bitLength(),
					(name.concat(modulus.toString())).hashCode() + "", pubkey.getClass().getName());
		} else {
			publicAlias = new KeyStoreAlias(name, KeyType.KEYPAIR_PUBLIC_KEY, "MpRSA",
					new BigInteger(modulus.toString()).bitLength(), (name.concat(modulus.toString())).hashCode() + "",
					pubkey.getClass().getName());
		}
		...
		final MpRSAPrivateKey privkey = new MpRSAPrivateKey(n, e, new FlexiBigInt(privExponent),
				new FlexiBigInt(firstPrime), new FlexiBigInt(secondPrime), FlexiBigInt.ZERO, FlexiBigInt.ZERO,
				FlexiBigInt.ZERO, otherPrimeInfo);
		final KeyStoreAlias privateAlias;
		if (crackedBy != null) {
			privateAlias = new KeyStoreAlias(name, KeyType.KEYPAIR_PRIVATE_KEY,
					"MpRSA" + " (cracked by: " + crackedBy + ") ", new BigInteger(modulus.toString()).bitLength(),
			...
			privateAlias = new KeyStoreAlias(name, KeyType.KEYPAIR_PRIVATE_KEY, "MpRSA",
					new BigInteger(modulus.toString()).bitLength(), (name.concat(modulus.toString())).hashCode() + "",
					privkey.getClass().getName());
		...
		while (aliases != null && aliases.hasMoreElements()) {
			...
			if (alias.getClassName().equals(RSAPublicKey.class.getName())
				...
				if (!alias.getOperation().contains(identity)) {
					...
					pubkeys.put(alias.getContactName()
							+ Messages.IdentityManager_15 + alias
									.getKeyLength()
							+ Messages.IdentityManager_16 + operation[0] + Messages.IdentityManager_17
							+ getKeyIDFromDB(alias) + Messages.IdentityManager_23 + Messages.IdentityManager_22
							+ alias.getOperation().substring(alias.getOperation().lastIndexOf(':') + 2,
									alias.getOperation().lastIndexOf(')')),
							alias);
				}
			}
		}
	...
	public TreeMap<String, KeyStoreAlias> loadAllKeysForIdentityAndOtherPublics(String identity) {
		...
		while (aliases != null && aliases.hasMoreElements()) {
			alias = new KeyStoreAlias(aliases.nextElement());
			// load RSA public and private keys for a certain identity and all other public
			// keys for other
			...
			if (((alias.getClassName().equals(RSAPublicKey.class.getName())
					...
					|| alias.getClassName().equals(RSAPublicKey.class.getName())
							&& !alias.getContactName().equals(identity)
					|| alias.getOperation().contains(identity)) {
				String pairPart = "";
				/**
				 * print the available keys in the following style: 1a – Alice Whitehat –
				 * MpPrivateKey 1b – Alice Whitehat – RSAPublicKey 2 – Bob Whitehat –
				 * RSAPublicKey 3 – Eve Whitehat – RSAPublicKey 4a – Eve Whitehat – MpPrivateKey
				 * (cracked by Alice Whitehat) 4b – Eve Whitehat – RSAPublicKey (cracked by
				 * Alice Whitehat)
				 */
				...
				if (alias.getClassName().contains("PublicKey") && alias.getOperation().equals("MpRSA")) {
					keytype = "MpRSAPublicKey";
						...
						|| (alias.getClassName().toString().contains("PublicKey")
								&& alias.getOperation().contains("RSA"))) {
				...
				if (alias.getOperation().contains(identity)) {
					keys.put(getKeyIDFromDB(alias)
							+ pairPart + Messages.IdentityManager_23 + alias
									.getContactName()
							+ Messages.IdentityManager_23 + alias.getKeyLength() + Messages.IdentityManager_24 + keytype
							+ Messages.IdentityManager_23 + Messages.IdentityManager_22
							+ alias.getOperation().substring(alias.getOperation().lastIndexOf(':') + 2,
									alias.getOperation().lastIndexOf(')')),
							alias);
				} else {
	 ...
	 * @return the parameters in an Vector<String> in the following order:
	 *         algorithm, format, e, N
	 */
	public Vector<String> getAllRSAPubKeyParameters(KeyStoreAlias alias) {
		Vector<String> parameters = new Vector<String>();
		try {
			java.security.PublicKey x = KeyStoreManager.getInstance().getCertificate(alias).getPublicKey();

			if (x instanceof RSAPublicKey) {
				// Flexiprovider generated key
				RSAPublicKey pubkey = (RSAPublicKey) x;
			...
			} else if (x instanceof BCRSAPublicKey) {
				// Bouncycastle generated key
				BCRSAPublicKey pubkey = (BCRSAPublicKey) x;
				parameters.add(pubkey.getAlgorithm());
				parameters.add(pubkey.getFormat());
				parameters.add(pubkey.getPublicExponent().toString());
				parameters.add(pubkey.getModulus().toString());
			}
	 ...
	 * @param password the password for the key
	 * @return the parameters in an Vector<String> in the following order:
	 *         algorithm, format, p, q, other Primes, d, e, N
	 */
	 ...
	 * @return parameters: contains the publicKey Parameters. The first parameter is
	 *         'N', the second 'e'
	 */
			...
			if ((alias.getClassName().equals(RSAPrivateCrtKey.class.getName())
				...
				if (!alias.getOperation().contains(identityName)) {
					...
					keyStoreItems.put(alias.getContactName()
							+ Messages.IdentityManager_26 + alias.getKeyLength() + Messages.IdentityManager_27 + keytype
							+ Messages.IdentityManager_28 + getKeyIDFromDB(alias) + Messages.IdentityManager_23
							+ Messages.IdentityManager_22 + alias.getOperation().substring(
									alias.getOperation().lastIndexOf(':') + 2, alias.getOperation().lastIndexOf(')')),
							alias);
				}
			}
		}
