...
//                  Brute Force for up to 4 x 4 grilles. Creates all possibles grilles and evaluates each text decrypted by them
					...
					LogUtil.logInfo(Activator.PLUGIN_ID,
							Messages.MethodApplication_output_finishedProcedure + procedure);
				} else {
//                  Hill-Cimbing for templates from 5 x 5. creates random grilles and evaluates those and slightly variations of them before
//                  trying new random grilles.
					procedure = Messages.MethodApplication_procedure_hillClimbing;
...
//              sets value in relation to size
				...
				if (relativeValue < tempRelativeValue) {
					fwAnalysisOutput += Messages.MethodApplication_output_relativeValue
							+ myRound(relativeValue/* , 2 */) + Messages.MethodApplication_output_relativeTo
							+ myRound(alltimeLow/* , 2 */) + Messages.MethodApplication_output_length + length;
					LogUtil.logInfo(Activator.PLUGIN_ID,
							Messages.MethodApplication_output_relativeValue + myRound(relativeValue/* , 2 */)
									+ Messages.MethodApplication_output_relativeTo + myRound(alltimeLow/* , 2 */)
									+ Messages.MethodApplication_output_length + length);
...
//                              try other 3 positions of every hole in grille
				...
				if (min < oldValue) {
					...
					fwAnalysisOutput += Messages.MethodApplication_output_try + iAll
							+ Messages.MethodApplication_output_changes + changes + " ("
