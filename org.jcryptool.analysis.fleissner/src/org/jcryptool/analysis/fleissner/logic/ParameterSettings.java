...
public class ParameterSettings {
	...
	/**
	 * Sets all parameters and method to be applied. Checks, if combinations of
	 * given parameters are useful to start a method. Sets default values to non
	 * given parameters, if useful and throws Exception else.
	 * 
		...
		if (args.length > 0) {
			...
			for (int i = 0; i < args.length; i += 2) {
				...
				case "-method": //$NON-NLS-1$
					if ((args[i + 1].equals("encrypt")) || (args[i + 1].equals("analyze")) //$NON-NLS-1$ //$NON-NLS-2$
							|| (args[i + 1].equals("decrypt"))) { //$NON-NLS-1$
						method = args[i + 1];
...
//						there is no valid key with length smaller than 2
						...
						LogUtil.logInfo(Messages.ParameterSettings_info_keyLength + templateLength
								+ Messages.ParameterSettings_info_holes + holes);
					}
					break;
				case "-key": //$NON-NLS-1$
					/**
					 * sets key in the following shape (example for 3x3 grille): 0,0,2,1 for two
					 * holes at coordinates (0,0) and (2,1) and calulates and sets key length and
					 * holes
					 */
					try {
						...
						LogUtil.logInfo(Messages.ParameterSettings_info_key + template
								+ Messages.ParameterSettings_info_keyLength2 + String.valueOf(templateLength)
								+ Messages.ParameterSettings_info_holes + String.valueOf(holes));
					} catch (Exception e) {
